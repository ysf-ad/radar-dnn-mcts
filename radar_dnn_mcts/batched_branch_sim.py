...
class BatchedRootBranchSimulator:
    ...
    def restore_root(self, snapshot=None) -> None:
        snap = self.snapshot_root() if snapshot is None else snapshot
        binding.vec_restore_all(self.env, snap)

    def step_actions(self, actions: np.ndarray, snapshot=None) -> BranchStepResult:
        ...
        observations = [get_obs_from_buf(self.obs_buf[i], max_trackers=self.max_trackers) for i in range(actions.size)]
