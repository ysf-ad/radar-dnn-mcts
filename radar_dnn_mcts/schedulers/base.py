...
def choose_row(
    ...
    """Select the highest weighted policy/value score among valid rows."""
...
def selected_mask(
    ...
    """Mask targets already used or expired at the current schedule time."""
...
def update_prefix(row: int, obs: dict, elapsed: float, searches: int, tracks: int, selected: set[int]):
    """Update deterministic prefix bookkeeping after selecting one row."""
