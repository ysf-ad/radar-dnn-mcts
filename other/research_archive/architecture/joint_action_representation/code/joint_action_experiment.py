...
def execute_first_valid_action_joint(eng, plan, remaining_ms: float):
    ...
    for a in plan:
        ...
        if not bool(int(obs_before.get("enable_x_band", 0))):
            # In S-only experiments a joint action is represented internally as
            # (S action, X placeholder).  The C environment does not consume the
            # encoded joint integer as that pair, so execute the S action through
            # the normal single-action path.
            return execute_first_valid_action(eng, [int(s_action)], remaining_ms)
