            ...
            for rate in parse_floats(args.rates):
                ...
                for name, planner in planners.items():
                    ...
                    row = {
                        ...
                        "tracked": float(s.get("tracked_targets", np.nan)),
                        "drop": float(s.get("drop_pct_active", np.nan)),
                        "delay": float(s.get("mean_delay_active", np.nan)),
                        "s_util": float(s.get("s_util", np.nan)),
                        "x_util": float(s.get("x_util", np.nan)),
                        "mean_sensor_util": float(s.get("mean_sensor_util", np.nan)),
                        "both_sensor_util": float(s.get("both_sensor_util", np.nan)),
                        "latency_ms_window": float(1000.0 * seconds / max(1, int(args.eval_windows))),
                        "windows_completed": int(args.eval_windows),
                    }
                    rows.append(row)
...
def main():
    ...
    if str(args.state_in).strip():
        model = ActionAttentionFactorizedNet(int(args.d_model), int(args.nhead), int(args.nlayers))
        state = torch.load(str(args.state_in), map_location="cpu", weights_only=False)
        if isinstance(state, dict) and "state_dict" in state:
            state = state["state_dict"]
        model.load_state_dict(state, strict=True)
        model.eval()
        print({"loaded_state": str(args.state_in)}, flush=True)
    else:
    ...
    metric_cols = [c for c in ["tracked", "drop", "delay", "s_util", "x_util", "mean_sensor_util", "both_sensor_util"] if c in raw.columns]
    if metric_cols:
        summary = (
            raw.groupby("method")
            .agg(
                ...
                drop=("drop", "mean"),
                delay=("delay", "mean"),
                latency_ms_window=("latency_ms_window", "mean"),
                n=("reward", "size"),
            )
            .reset_index()
            .sort_values("reward", ascending=False)
