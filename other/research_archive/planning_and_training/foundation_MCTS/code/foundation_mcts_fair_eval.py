...
def run_heuristic(planner, name: str, initial: int, seed: int, windows: int, env_cfg: dict):
    ...
    return run_fixed(planner, name, int(initial), MAXT, int(seed), int(windows), 200, engine_env_cfg(env_cfg))
    ...
    for seed in parse_ints(args.seeds):
        for initial in parse_ints(args.initials):
            for rate in parse_floats(args.rates):
                ...
                env_cfg["enable_x_band"] = 0 if bool(getattr(args, "single_sensor", False)) else 1
...
def main():
    ...
    if args.mode == "physical_targets":
        ...
        return
    state_path = args.state if str(args.state).strip() and args.state.is_file() else None
    model = load_foundation(args.ckpt, state_path)
    variant_cfg = {
