    ...
    payload = [
        {
            ...
            "pair_pi": getattr(t, "pair_pi", None),
        }
        for t in targets
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for item in payload:
        ...
        targets.append(
            SearchTarget(
                ...
                pair_pi=None if item.get("pair_pi", None) is None else np.asarray(item["pair_pi"], dtype=np.float32),
            )
        )
    return targets
...
def base_exact_args(args0) -> SimpleNamespace:
    zero_action_modes = {"penalty_only_frame", "pufferlib_service"}
    zero_action_rewards = bool(getattr(args0, "zero_action_rewards", False)) or str(getattr(args0, "env_mode", "")) in zero_action_modes
