...
from exact_env_mutual import shaped_step_reward
...
def xs_busy_action(max_trackers: int = MAXT) -> int:
    """No-op filler for an already-busy sensor slot inside a joint action."""
    return int(max_trackers) + 1


def pending_busy_row(base: int | float | None) -> int:
    """Encode an in-flight action base as a negative BUSY row for pair heads."""
    if base is None:
        return -1
    base_i = int(base)
    if base_i < 0:
...
def _sensor_prefix(sensor: int) -> str:
    return "s" if int(sensor) == 0 else "x"


def _clear_pending_fields(obs: dict, sensor: int) -> None:
    prefix = _sensor_prefix(sensor)
    obs[f"{prefix}_pending_base"] = -1
    obs[f"{prefix}_pending_total_ms"] = 0.0
    obs[f"{prefix}_pending_remaining_ms"] = 0.0


def _set_pending_fields(obs: dict, sensor: int, base: int, total_ms: float, remaining_ms: float) -> None:
    prefix = _sensor_prefix(sensor)
    remaining = max(0.0, float(remaining_ms))
    if remaining <= 1e-6:
        _clear_pending_fields(obs, sensor)
        return
    obs[f"{prefix}_pending_base"] = int(base)
    obs[f"{prefix}_pending_total_ms"] = max(1.0, float(total_ms))
    obs[f"{prefix}_pending_remaining_ms"] = remaining


def decoded_sensor_log_fields(atoms, atom_executed):
    fields = {
        "s_action": -1,
        "x_action": -1,
        "s_base": -1,
    ...
    for atom, done in zip(atoms, atom_executed):
        base, sensor = xs_decode_action(int(atom), MAXT)
        if sensor is None or int(sensor) not in {0, 1}:
            continue
        prefix = "s" if int(sensor) == 0 else "x"
        fields[f"{prefix}_action"] = int(atom)
        fields[f"{prefix}_base"] = int(base)
        fields[f"{prefix}_executed"] = bool(done)
        if not bool(done):
            fields[f"{prefix}_type"] = "Busy"
        elif int(base) == 0:
            fields[f"{prefix}_type"] = "Search"
        elif int(base) > 0:
            fields[f"{prefix}_type"] = "Track"
        else:
            fields[f"{prefix}_type"] = "Invalid"
    return fields


def execute_plan_until_budget_joint_compatible(
    eng,
    plan,
    budget_ms: float,
    search_debt_ms: float,
    planner_name: str,
    seed: int,
    window_idx: int,
    ...
    for action in plan:
        ...
        if observer is not None and hasattr(observer, "observe_executed"):
            observer.observe_executed(obs_before, int(executed_action), float(dt))
        if env_cfg is not None:
            reward = shaped_step_reward(float(reward), float(dt), obs_before, obs_after, env_cfg, int(executed_action))
        ...
        decoded = decoded_sensor_log_fields(atoms, atom_executed)
        if float(obs_before.get("s_band_busy_ms", 0.0)) > 0.0 and int(decoded.get("s_action", -1)) < 0:
            decoded["s_type"] = "Busy"
        if float(obs_before.get("x_band_busy_ms", 0.0)) > 0.0 and int(decoded.get("x_action", -1)) < 0:
            decoded["x_type"] = "Busy"
        if any(is_search):
            search_debt_ms = 0.0
        else:
        ...
        rows.append(
            {
                ...
                "s_base": -1,
                "x_base": -1,
                "s_type": "Busy" if s_busy else "Idle",
                "x_type": "Busy" if x_busy else "Idle",
                "s_executed": False,
                "x_executed": False,
                "action_type": "Wait",
                "reward": float(reward),
...
class AsyncCoupledJointPlanner:
    ...
    def _ranked_for_sensor(self, obs: dict, sensor: int, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        return self._ranked_for_sensors(
            obs,
            [int(sensor)],
            selected,
            elapsed,
    ...
    def _ranked_for_sensors(
        ...
        branch_pack = None
        if os.environ.get("RADARXS_BRANCH_FACTOR_INFERENCE", "0").strip() == "1" and hasattr(self.base, "branch_action_scores"):
            branch_pack = self.base.branch_action_scores(
                local_obs,
                selected=selected,
                elapsed=elapsed,
                search_count=search_count,
        ...
        if branch_pack is not None:
            branch, target_scores, track_mask = branch_pack
            wanted = {int(sensor) for sensor in sensors}
            ranked = {int(sensor): [] for sensor in wanted}
            for sensor in wanted:
                search_action = xs_s_search_action(MAXT) if sensor == 0 else xs_x_search_action(MAXT)
                search_score = float(branch[int(sensor), 0])
                track_branch_score = float(branch[int(sensor), 1])
                track_items = []
                for action in physical_candidates(local_obs, top_k=MAXT):
                    ...
                    if int(action_sensor) != int(sensor) or int(base) <= 0:
                        continue
                    if int(base) in selected or int(base) >= len(track_mask) or not bool(track_mask[int(base)]):
                        continue
                    track_items.append((track_branch_score + float(target_scores[int(base), int(sensor)]), int(base), int(action)))
                track_items.sort(reverse=True, key=lambda x: x[0])
                if not track_items or search_score >= track_branch_score:
                    out = [(search_score, 0, int(search_action))]
                else:
                    out = track_items[: self.per_sensor_top]
                    if self.include_search_candidate:
                        out.append((search_score, 0, int(search_action)))
                ranked[sensor] = out
            return ranked
        scores = self.base.score_actions(
            local_obs,
        ...
        for action in physical_candidates(local_obs, top_k=MAXT):
            base, action_sensor = xs_decode_action(int(action), MAXT)
            if int(base) < 0 or int(action_sensor) not in wanted:
                continue
            if int(base) > 0 and int(base) in selected:
                continue
            sensor = int(action_sensor)
            ranked[sensor].append((float(scores[int(base), sensor]), int(base), int(action)))
        for sensor in wanted:
            ranked[sensor].sort(reverse=True, key=lambda x: x[0])
            out = ranked[sensor][: self.per_sensor_top]
            search_action = xs_s_search_action(MAXT) if sensor == 0 else xs_x_search_action(MAXT)
            if self.include_search_candidate and not any(int(action) == int(search_action) for _score, _base, action in out):
                for item in ranked[sensor]:
    ...
    def warmup(self, obs: dict):
        if getattr(self.base, "use_traced_forward", False):
            self._ranked_for_sensors(
                obs,
                [0, 1],
                set(),
                0.0,
    ...
    def _candidate_pairs(self, obs: dict, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        ...
        free_sensors = ([0] if s_free else []) + ([1] if x_free else [])
        ranked_by_sensor = self._ranked_for_sensors(
            obs,
            free_sensors,
            selected,
            elapsed,
            ...
            track_count,
            last,
        ) if free_sensors else {}
        s_ranked = ranked_by_sensor.get(0, [])
        x_ranked = ranked_by_sensor.get(1, [])
        busy_token = xs_busy_action(MAXT)
        out = []
        ...
        if s_ranked:
            for s_score, _s_base, s_action in s_ranked:
                out.append((float(s_score), encode_joint_action(int(s_action), int(busy_token))))
        if x_ranked:
            for x_score, _x_base, x_action in x_ranked:
                out.append((float(x_score), encode_joint_action(int(busy_token), int(x_action))))
        if s_busy > 0.0 or (x_enabled and x_busy > 0.0):
            out.append((-1e6, encode_joint_action(int(busy_token), int(busy_token))))
        deduped = {}
    ...
    def _advance_synthetic(self, obs: dict, action: int, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        ...
        started: list[tuple[int, int, float]] = []
        for atom in atoms:
            base, sensor = xs_decode_action(int(atom), MAXT)
            if sensor is None:
                continue
            if int(sensor) == 0 and s_busy <= 0.0:
                ...
                started.append((0, int(base), float(s_busy)))
            elif int(sensor) == 1 and x_busy <= 0.0:
                x_busy = self._duration(obs, int(atom))
                started.append((1, int(base), float(x_busy)))
            else:
                continue
            if int(base) == 0:
                search_count += 1
        ...
        for sensor in (0, 1):
            prefix = _sensor_prefix(sensor)
            prior_base = int(obs.get(f"{prefix}_pending_base", -1))
            prior_remaining = float(obs.get(f"{prefix}_pending_remaining_ms", obs.get(f"{prefix}_band_busy_ms", 0.0)))
            prior_total = float(obs.get(f"{prefix}_pending_total_ms", max(1.0, prior_remaining)))
            if prior_base >= 0 and prior_remaining > 0.0:
                _set_pending_fields(obs, sensor, prior_base, prior_total, prior_remaining - dt)
            else:
                _clear_pending_fields(obs, sensor)
        for sensor, base, total in started:
            _set_pending_fields(obs, sensor, int(base), float(total), float(total) - dt)
...
class OnePassWorkConservingPlanner(WorkConservingAsyncCoupledPlanner):
    """Score once at the window start, then fill the window from static S/X rankings."""

    def _static_rankings(
        self,
        obs: dict,
        selected: set[int],
        elapsed: float,
        ...
        for action in physical_candidates(obs, top_k=MAXT):
            base, sensor = xs_decode_action(int(action), MAXT)
            if int(base) < 0 or int(sensor) not in {0, 1}:
                continue
            ranked[int(sensor)].append((float(scores[int(base), int(sensor)]), int(base), int(action)))
        for sensor in ranked:
            ranked[sensor].sort(reverse=True, key=lambda x: x[0])
            search_action = xs_s_search_action(MAXT) if sensor == 0 else xs_x_search_action(MAXT)
            if self.include_search_candidate and not any(int(action) == int(search_action) for _score, _base, action in ranked[sensor]):
                ranked[sensor].append((float(scores[0, sensor]), 0, int(search_action)))
        return obs, ranked

    @staticmethod
    def _first_available(ranked: list[tuple[float, int, int]], selected: set[int], blocked_base: int | None = None):
        for score, base, action in ranked:
            if int(base) > 0 and int(base) in selected:
                continue
            if blocked_base is not None and int(base) > 0 and int(base) == int(blocked_base):
                continue
            return float(score), int(base), int(action)
        return None

    def plan(self, obs, budget_ms=200):
        plan_obs, ranked = self._static_rankings(obs, set(), 0.0, 0, 0, -1)
        selected: set[int] = set()
        plan = []
        elapsed = 0.0
        while elapsed < float(budget_ms) and len(plan) < 128:
            s_item = self._first_available(ranked[0], selected)
            x_item = self._first_available(ranked[1], selected, blocked_base=s_item[1] if s_item is not None else None)
            if s_item is None and x_item is None:
                break
            s_action = s_item[2] if s_item is not None else xs_s_search_action(MAXT)
            x_action = x_item[2] if x_item is not None else xs_x_search_action(MAXT)
            action = encode_joint_action(int(s_action), int(x_action))
            plan.append(int(action))
            selected, elapsed, _search_count, _track_count, _last = self._advance_synthetic(
                plan_obs,
                int(action),
                selected,
                elapsed,
...
class ChunkedWorkConservingPlanner(OnePassWorkConservingPlanner):
    """Re-score every fixed number of planned joint actions."""

    def __init__(self, *args, chunk_size: int = 4, **kwargs):
    ...
    def plan(self, obs, budget_ms=200):
        ...
        last = -1
        while elapsed < float(budget_ms) and len(plan) < 128:
            _obs, ranked = self._static_rankings(plan_obs, selected, elapsed, search_count, track_count, last)
            made_progress = False
            for _ in range(self.chunk_size):
                if elapsed >= float(budget_ms) or len(plan) >= 128:
                    break
                s_item = self._first_available(ranked[0], selected)
                x_item = self._first_available(ranked[1], selected, blocked_base=s_item[1] if s_item is not None else None)
                if s_item is None and x_item is None:
...
class SearchCountCachedWorkConservingPlanner(OnePassWorkConservingPlanner):
    """Cache per-search-count rankings once per window.

    Full work-conserving planning re-scores after every synthetic action.  The
    largest discrete state change for search/track balance is search_count, so
    this planner precomputes a small table of score rankings indexed by
    search_count and then greedily rolls the window using that table.
    """

    def __init__(self, *args, max_cached_search_count: int = 8, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_cached_search_count = max(0, int(max_cached_search_count))

    def _cached_rankings(self, obs: dict):
        plan_obs = attach_env_obs(dict(obs), self.env_cfg, True, True)
        cache = {}
        for search_count in range(self.max_cached_search_count + 1):
            _obs, ranked = self._static_rankings(plan_obs, set(), 0.0, int(search_count), 0, -1)
            cache[int(search_count)] = ranked
        return plan_obs, cache

    def plan(self, obs, budget_ms=200):
        plan_obs, cache = self._cached_rankings(obs)
        selected: set[int] = set()
        plan = []
        elapsed = 0.0
        search_count = 0
        ...
        last = -1
        while elapsed < float(budget_ms) and len(plan) < 128:
            key = min(int(search_count), self.max_cached_search_count)
            ranked = cache[key]
            s_item = self._first_available(ranked[0], selected)
            x_item = self._first_available(ranked[1], selected, blocked_base=s_item[1] if s_item is not None else None)
            if s_item is None and x_item is None:
...
class PairHeadWorkConservingPlanner(WorkConservingAsyncCoupledPlanner):
    """Work-conserving planner that scores free S/X pairs with the learned pair head."""

    def __init__(
        self,
        model,
        env_cfg: dict,
        variant: str = "two_row_pair_action_attention",
        per_sensor_top: int = 6,
        policy_weight: float = 1.0,
        q_weight: float = 0.0,
        search_score_bias: float = 0.0,
        include_search_candidate: bool = True,
        full_pair_candidates: bool = False,
        score_busy_candidates: bool = False,
    ):
        ...
        self.adapt = adapter()
        self.full_pair_candidates = bool(full_pair_candidates)
        self.score_busy_candidates = bool(score_busy_candidates)
        self.pending = {0: None, 1: None}

    def reset(self):
        self.pending = {0: None, 1: None}

    def _attach_pending(self, obs: dict) -> dict:
        out = dict(obs)
        for sensor in (0, 1):
            prefix = _sensor_prefix(sensor)
            busy_ms = float(out.get(f"{prefix}_band_busy_ms", 0.0))
            pending = self.pending.get(sensor)
            if busy_ms > 0.0 and pending is not None:
                _set_pending_fields(
                    out,
                    sensor,
                    int(pending.get("base", -1)),
                    float(pending.get("total_ms", max(1.0, busy_ms))),
                    busy_ms,
                )
            elif busy_ms <= 0.0:
                self.pending[sensor] = None
                _clear_pending_fields(out, sensor)
            else:
                _clear_pending_fields(out, sensor)
        return out

    def observe_executed(self, obs_before: dict, executed_action: int, dt_ms: float) -> None:
        dt = max(0.0, float(dt_ms))
        for sensor in (0, 1):
            pending = self.pending.get(sensor)
            if pending is not None:
                pending["remaining_ms"] = max(0.0, float(pending.get("remaining_ms", 0.0)) - dt)
                if pending["remaining_ms"] <= 1e-6:
                    self.pending[sensor] = None
        ...
        for atom in atoms:
            ...
            if sensor not in {0, 1} or int(base) < 0:
                continue
            prefix = _sensor_prefix(int(sensor))
            if float(obs_before.get(f"{prefix}_band_busy_ms", 0.0)) > 0.0:
                continue
            total = self._duration(obs_before, int(atom))
            remaining = max(0.0, float(total) - dt)
            self.pending[int(sensor)] = (
                {"base": int(base), "total_ms": float(total), "remaining_ms": remaining}
                if remaining > 1e-6
                else None
            )

    def _busy_pair_row(self, obs: dict, sensor: int) -> int:
        prefix = _sensor_prefix(sensor)
        base = int(obs.get(f"{prefix}_pending_base", -1))
        return pending_busy_row(base)

    def _state_tensors(self, obs: dict, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        local_obs = attach_env_obs(self._attach_pending(obs), self.env_cfg, True, False)
        tok = tokenize(self.adapt, local_obs, selected=set(selected), search_count=int(search_count))
        slot = slot_features(local_obs, float(elapsed), int(search_count), int(track_count), int(last), 200.0)
        return local_obs, tok, slot

    def _score_pair_rows(self, scores_t, q_t, action_ctx_t, valid_t, slot_t, pair_rows: list[tuple[int, int]]):
        if not pair_rows:
            return np.asarray([], dtype=np.float32), np.asarray([], dtype=bool)
        rows_t = torch.tensor(pair_rows, dtype=torch.long).unsqueeze(0)
        with torch.inference_mode():
            if hasattr(self.model, "action_feature_proj"):
                _scores, _q, pair_scores_t, pair_q_t, pair_valid_t = self.model.forward_pair_candidates_from_context(
                    scores_t,
                    q_t,
                    action_ctx_t,
                    valid_t,
            ...
            pair_scores_np = (self.policy_weight * pair_scores_t + self.q_weight * pair_q_t).squeeze(0).cpu().numpy()
            pair_valid_np = pair_valid_t.squeeze(0).cpu().numpy().astype(bool)
        return pair_scores_np, pair_valid_np

    def _pair_score_arrays(self, obs: dict, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        local_obs, tok, slot = self._state_tensors(obs, selected, elapsed, search_count, track_count, last)
        with torch.inference_mode():
            x = torch.from_numpy(tok).float().unsqueeze(0)
            s = torch.from_numpy(slot).float().unsqueeze(0)
            scores, q, pair_scores, pair_q, pair_valid = self.model.forward_pair_scores(x, s)
            marginal = (self.policy_weight * scores + self.q_weight * q).squeeze(0).cpu().numpy()
            pair = (self.policy_weight * pair_scores + self.q_weight * pair_q).squeeze(0).cpu().numpy()
            valid = pair_valid.squeeze(0).cpu().numpy().astype(bool)
        marginal = np.asarray(marginal, dtype=np.float32).copy()
        marginal[0, :] += self.search_score_bias
        pair = np.asarray(pair, dtype=np.float32).copy()
        pair[0, :] += self.search_score_bias
        pair[:, 0] += self.search_score_bias
        return local_obs, marginal, pair, valid

    def _ranked_for_sensor_from_scores(self, obs: dict, sensor: int, selected: set[int], scores: np.ndarray):
        ranked = []
        for action in physical_candidates(obs, top_k=MAXT):
            base, action_sensor = xs_decode_action(int(action), MAXT)
            if int(action_sensor) != int(sensor) or int(base) < 0:
    ...
    def _candidate_pairs(self, obs: dict, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        obs_attached, tok, slot = self._state_tensors(obs, selected, elapsed, search_count, track_count, last)
        with torch.inference_mode():
            x = torch.from_numpy(tok).float().unsqueeze(0)
            s = torch.from_numpy(slot).float().unsqueeze(0)
            scores_t, q_t, action_ctx_t, valid_t = self.model._base_action_context(x, s)
            marginal = (self.policy_weight * scores_t + self.q_weight * q_t).squeeze(0).cpu().numpy()
        marginal = np.asarray(marginal, dtype=np.float32).copy()
        marginal[0, :] += self.search_score_bias
        ...
        s_free = s_busy <= 0.0
        x_free = x_enabled and x_busy <= 0.0
        s_ranked = self._ranked_for_sensor_from_scores(obs_attached, 0, selected, marginal) if s_free else []
        x_ranked = self._ranked_for_sensor_from_scores(obs_attached, 1, selected, marginal) if x_free else []
        busy_token = xs_busy_action(MAXT)
        out = []
        if s_ranked and x_ranked and self.full_pair_candidates:
            _obs_full, _marginal_full, pair_full, valid_full = self._pair_score_arrays(
                obs_attached,
                selected,
                elapsed,
                search_count,
            ...
            for action in physical_candidates(obs_attached, top_k=MAXT):
                base, action_sensor = xs_decode_action(int(action), MAXT)
                if int(base) < 0 or (int(base) > 0 and int(base) in selected):
                    continue
                if int(action_sensor) == 0:
                    s_actions[int(base)] = int(action)
                elif int(action_sensor) == 1:
                    x_actions[int(base)] = int(action)
            pair_scores = []
            for s_base, s_action in s_actions.items():
                for x_base, x_action in x_actions.items():
                    if int(s_base) > 0 and int(s_base) == int(x_base):
                        continue
                    if int(s_base) >= pair_full.shape[0] or int(x_base) >= pair_full.shape[1]:
                        continue
                    if not bool(valid_full[int(s_base), int(x_base)]):
                        continue
                    pair_scores.append((float(pair_full[int(s_base), int(x_base)]), encode_joint_action(int(s_action), int(x_action))))
            out.extend(pair_scores)
        elif s_ranked and x_ranked:
            pair_rows = []
            pair_actions = []
            for _s_score, s_base, s_action in s_ranked:
                for _x_score, x_base, x_action in x_ranked:
                    if int(s_base) > 0 and int(s_base) == int(x_base):
                        continue
                    pair_rows.append((int(s_base), int(x_base)))
                    pair_actions.append(encode_joint_action(int(s_action), int(x_action)))
            if pair_rows:
                pair_scores_np, pair_valid_np = self._score_pair_rows(scores_t, q_t, action_ctx_t, valid_t, s, pair_rows)
                for pair_idx, action in enumerate(pair_actions):
                    if bool(pair_valid_np[pair_idx]):
                        out.append((float(pair_scores_np[pair_idx]), int(action)))
        elif s_ranked:
            if self.score_busy_candidates:
                pair_rows = []
                pair_actions = []
                x_busy_row = self._busy_pair_row(obs_attached, 1)
                for _s_score, _s_base, s_action in s_ranked:
                    pair_rows.append((int(_s_base), int(x_busy_row)))
                    pair_actions.append(encode_joint_action(int(s_action), int(busy_token)))
                pair_scores_np, pair_valid_np = self._score_pair_rows(scores_t, q_t, action_ctx_t, valid_t, s, pair_rows)
                for pair_idx, action in enumerate(pair_actions):
                    if bool(pair_valid_np[pair_idx]):
                        out.append((float(pair_scores_np[pair_idx]), int(action)))
            ...
            if self.score_busy_candidates:
                pair_rows = []
                pair_actions = []
                s_busy_row = self._busy_pair_row(obs_attached, 0)
                for _x_score, _x_base, x_action in x_ranked:
                    pair_rows.append((int(s_busy_row), int(_x_base)))
                    pair_actions.append(encode_joint_action(int(busy_token), int(x_action)))
                pair_scores_np, pair_valid_np = self._score_pair_rows(scores_t, q_t, action_ctx_t, valid_t, s, pair_rows)
                for pair_idx, action in enumerate(pair_actions):
                    if bool(pair_valid_np[pair_idx]):
                        out.append((float(pair_scores_np[pair_idx]), int(action)))
...
class PairHeadNoBusyJointPlanner(PairHeadWorkConservingPlanner):
    """Fully joint pair scorer with no single-free-sensor BUSY actions."""

    def _candidate_pairs(self, obs: dict, selected: set[int], elapsed: float, search_count: int, track_count: int, last: int):
        obs_attached = attach_env_obs(dict(obs), self.env_cfg, True, True)
        s_free = float(obs_attached.get("s_band_busy_ms", 0.0)) <= 0.0
        x_free = bool(int(obs_attached.get("enable_x_band", 0))) and float(obs_attached.get("x_band_busy_ms", 0.0)) <= 0.0
        if s_free and x_free:
            return super()._candidate_pairs(obs, selected, elapsed, search_count, track_count, last)
        busy_token = xs_busy_action(MAXT)
        return [(-1e6, encode_joint_action(int(busy_token), int(busy_token)))]
...
class DirectPlanAdapter:
    ...
    def warmup(self, obs):
        if hasattr(self.planner, "warmup"):
            self.planner.warmup(obs)

    def observe_executed(self, obs_before: dict, executed_action: int, dt_ms: float) -> None:
        if hasattr(self.planner, "observe_executed"):
            self.planner.observe_executed(obs_before, int(executed_action), float(dt_ms))
        ...
        if not plan:
            ...
            if is_joint_action(primary):
                ...
                if int(sensor) == 0 and s_free:
                    x_action = self._model_fill(obs, 1, selected) if x_free else busy_token
                    action = encode_joint_action(int(primary), int(x_action))
                elif int(sensor) == 1 and x_free:
                    s_action = self._model_fill(obs, 0, selected) if s_free else busy_token
                    action = encode_joint_action(int(s_action), int(primary))
                elif x_free:
                    x_action = self._model_fill(obs, 1, set())
                    action = encode_joint_action(int(busy_token), int(x_action))
                elif s_free:
                    s_action = self._model_fill(obs, 0, set())
                    action = encode_joint_action(int(s_action), int(busy_token))
                else:
                    action = encode_joint_action(int(busy_token), int(busy_token))
        meta["planning_ms"] = float(meta.get("planning_ms", 0.0)) + float((time.perf_counter() - t0) * 1000.0)
    ...
    try:
        ...
        for window in range(int(windows)):
            ...
            reward, spent, debt, executed, searches, arows = execute_plan_until_budget_joint_compatible(
                eng, plan, 200.0, debt, name, int(seed), int(window), env_cfg, observer=planner
            )
    ...
    if load_state:
        model = train_head(
            ...
            SimpleNamespace(
                ...
                use_arrival_token_feature=bool(getattr(args, "use_arrival_token_feature", False)),
                search_calibration_weight=float(getattr(args, "search_calibration_weight", 0.0)),
                target_label_smoothing=float(getattr(args, "target_label_smoothing", 0.0)),
                marginal_type_loss_weight=float(getattr(args, "marginal_type_loss_weight", 1.0)),
                marginal_target_loss_weight=float(getattr(args, "marginal_target_loss_weight", 1.0)),
                balanced_type_loss=bool(getattr(args, "balanced_type_loss", False)),
                pair_loss_mode=str(getattr(args, "pair_loss_mode", "sampled")),
                pair_loss_weight=float(getattr(args, "pair_loss_weight", 0.65)),
                freeze_non_pair_heads=bool(getattr(args, "freeze_non_pair_heads", False)),
                log_every=1,
        ...
        if bool(getattr(args, "non_strict_load", False)):
            missing, unexpected = model.load_state_dict(state, strict=False)
            print({"loaded_state": load_state, "strict": False, "missing": list(missing), "unexpected": list(unexpected)}, flush=True)
        else:
            model.load_state_dict(state, strict=True)
        if bool(getattr(args, "continue_training", False)):
            ...
            model = train_head(variant, targets, train_args, torch.device("cpu"), model=model)
...
def main() -> None:
    ...
    ap.add_argument("--gate-state", default="")
    ap.add_argument("--gate-corrected-cells", default="20:3,20:4,40:2,60:3,60:4")
    ap.add_argument("--gate-policy-weight", type=float, default=None)
    ap.add_argument("--gate-q-weight", type=float, default=None)
    ap.add_argument("--gate-search-bias", type=float, default=None)
    ap.add_argument("--autoregressive-save-state", default="")
    ap.add_argument("--autoregressive-load-state", default="")
    ap.add_argument("--pair-save-state", default="")
    ap.add_argument("--pair-load-state", default="")
    ap.add_argument("--pair-variant", default="two_row_pair_action_attention")
    ...
    ap.add_argument("--pair-full-candidates", action="store_true")
    ...
    ap.add_argument("--cached-search-count", type=int, default=8)
    ...
    need_pair = (not method_filter) or bool(
        {"Best_pair_workconserving_direct_PQ", "Best_pair_receding_direct_PQ", "Best_pair_pending_busy_direct_PQ", "Best_pair_no_busy_joint_direct_PQ"} & method_filter
    )
    model = train_best(args)
    gate_model = None
    if str(args.gate_state).strip():
        gate_model = train_variant(args, str(getattr(args, "variant", "two_row_action_attention_factored_loss")), str(args.gate_state).strip())
    gate_corrected_cells = set()
    for item in str(args.gate_corrected_cells).split(","):
        item = item.strip()
        if not item:
            continue
        left, right = item.split(":", 1)
        gate_corrected_cells.add((int(left), float(right)))
    ...
    pair_model = None
    if need_pair:
        pair_args = SimpleNamespace(**vars(args))
        pair_args.save_state = str(args.pair_save_state)
        pair_model = train_variant(
            pair_args,
            str(getattr(args, "pair_variant", "two_row_pair_action_attention")),
            str(args.pair_load_state),
        )
    ...
    for initial in parse_ints(args.initials):
        for rate in parse_floats(args.rates):
            ...
            if bool(args.use_arrival_token_feature):
            ...
            gated_workconserving_base = None
            if gate_model is not None:
                gated_model = gate_model if (int(initial), float(rate)) in gate_corrected_cells else model
                gated_policy_weight = float(args.gate_policy_weight) if args.gate_policy_weight is not None and (int(initial), float(rate)) in gate_corrected_cells else float(args.policy_weight)
                gated_q_weight = float(args.gate_q_weight) if args.gate_q_weight is not None and (int(initial), float(rate)) in gate_corrected_cells else float(args.q_weight)
                gated_search_bias = float(args.gate_search_bias) if args.gate_search_bias is not None and (int(initial), float(rate)) in gate_corrected_cells else float(args.search_bias)
                gated_base = PhysicalHeadPlanner(
                    gated_model,
                    str(args.variant),
                    env_cfg,
                    policy_weight=gated_policy_weight,
                    q_weight=gated_q_weight,
                    search_score_bias=gated_search_bias,
                )
                gated_workconserving_base = WorkConservingAsyncCoupledPlanner(
                    gated_base,
            ...
            onepass_workconserving_base = OnePassWorkConservingPlanner(
                base,
                per_sensor_top=int(args.per_sensor_top),
                include_search_candidate=bool(args.force_search_candidate),
            )
            chunk2_workconserving_base = ChunkedWorkConservingPlanner(
                base,
                per_sensor_top=int(args.per_sensor_top),
                include_search_candidate=bool(args.force_search_candidate),
                chunk_size=2,
            )
            chunk4_workconserving_base = ChunkedWorkConservingPlanner(
                base,
                per_sensor_top=int(args.per_sensor_top),
                include_search_candidate=bool(args.force_search_candidate),
                chunk_size=4,
            )
            chunk8_workconserving_base = ChunkedWorkConservingPlanner(
                base,
                per_sensor_top=int(args.per_sensor_top),
                include_search_candidate=bool(args.force_search_candidate),
                chunk_size=8,
            )
            search_cached_workconserving_base = SearchCountCachedWorkConservingPlanner(
                base,
                per_sensor_top=int(args.per_sensor_top),
                include_search_candidate=bool(args.force_search_candidate),
                max_cached_search_count=int(args.cached_search_count),
            )
            ...
            pair_direct_base = None
            pair_no_busy_base = None
            pair_pending_busy_base = None
            if pair_model is not None:
                pair_direct_base = PairHeadWorkConservingPlanner(
                    pair_model,
                    env_cfg,
                    variant=str(getattr(args, "pair_variant", "two_row_pair_action_attention")),
                    per_sensor_top=int(args.per_sensor_top),
                    policy_weight=float(args.policy_weight),
                    q_weight=float(args.q_weight),
                    search_score_bias=float(args.search_bias),
                    include_search_candidate=bool(args.force_search_candidate),
                    full_pair_candidates=bool(args.pair_full_candidates),
                )
                pair_pending_busy_base = PairHeadWorkConservingPlanner(
                    pair_model,
                    env_cfg,
                    variant=str(getattr(args, "pair_variant", "two_row_pair_action_attention")),
                    per_sensor_top=int(args.per_sensor_top),
                    ...
                    include_search_candidate=bool(args.force_search_candidate),
                    full_pair_candidates=bool(args.pair_full_candidates),
                    score_busy_candidates=True,
                )
                pair_no_busy_base = PairHeadNoBusyJointPlanner(
                    pair_model,
                    env_cfg,
                    variant=str(getattr(args, "pair_variant", "two_row_pair_action_attention")),
                    per_sensor_top=int(args.per_sensor_top),
            ...
            planners = {
                ...
                "Best_onepass_workconserving_PQ": DirectPlanAdapter(onepass_workconserving_base),
                "Best_chunk2_workconserving_PQ": DirectPlanAdapter(chunk2_workconserving_base),
                "Best_chunk4_workconserving_PQ": DirectPlanAdapter(chunk4_workconserving_base),
                "Best_chunk8_workconserving_PQ": DirectPlanAdapter(chunk8_workconserving_base),
                "Best_searchcached_workconserving_PQ": DirectPlanAdapter(search_cached_workconserving_base),
                "Best_workconserving_cell_gated_PQ": DirectPlanAdapter(gated_workconserving_base) if gated_workconserving_base is not None else None,
                ...
                "Best_pair_workconserving_direct_PQ": DirectPlanAdapter(pair_direct_base) if pair_direct_base is not None else None,
                "Best_pair_pending_busy_direct_PQ": DirectPlanAdapter(pair_pending_busy_base) if pair_pending_busy_base is not None else None,
                "Best_pair_no_busy_joint_direct_PQ": DirectPlanAdapter(pair_no_busy_base) if pair_no_busy_base is not None else None,
                "Best_pair_receding_direct_PQ": StatefulRecedingDirectAdapter(pair_direct_base) if pair_direct_base is not None else None,
            ...
            for name, planner in planners.items():
                ...
                w_out = w.copy()
                if has_busy and "window" in a.columns and "window" in w_out.columns:
                    busy_by_window = (
                        a.assign(both_busy_ms=np.minimum(a["s_busy_ms"].to_numpy(), a["x_busy_ms"].to_numpy()))
                        .groupby("window", as_index=False)
                        .agg(
                            s_busy_ms=("s_busy_ms", "sum"),
                            x_busy_ms=("x_busy_ms", "sum"),
                            both_busy_ms=("both_busy_ms", "sum"),
                        )
                    )
                    w_out = w_out.merge(busy_by_window, on="window", how="left")
                    for col in ("s_busy_ms", "x_busy_ms", "both_busy_ms"):
                        w_out[col] = w_out[col].fillna(0.0)
                    window_denom = w_out["spent_ms"].where(w_out["spent_ms"].abs() > 1e-9, 200.0).astype(float)
                    w_out["s_util"] = w_out["s_busy_ms"] / window_denom
                    w_out["x_util"] = w_out["x_busy_ms"] / window_denom
                    w_out["mean_sensor_util"] = 0.5 * (w_out["s_util"] + w_out["x_util"])
                    w_out["both_sensor_util"] = w_out["both_busy_ms"] / window_denom
                else:
                    w_out["s_busy_ms"] = np.nan
                    w_out["x_busy_ms"] = np.nan
                    w_out["both_busy_ms"] = np.nan
                    w_out["s_util"] = np.nan
                    w_out["x_util"] = np.nan
                    w_out["mean_sensor_util"] = np.nan
                    w_out["both_sensor_util"] = np.nan
                row = {
                    "method": name,
                    "initial": int(initial),
                    "rate": float(rate),
                ...
                rows.append(row)
                windows.append(w_out.assign(method=name, initial=int(initial), rate=float(rate), seed=int(args.seed)))
                if not a.empty:
