...
# RADAR ADAPTER CHANGE: Ray is optional for the local reference runner.
try:
    import ray
...
# RADAR ADAPTER CHANGE: support package import without changing script use.
try:
    from . import models
except ImportError:
    import models
...
class MCTS:
    ...
    def run(
        ...
        for _ in range(self.config.num_simulations):
            ...
            # WINDOWED CHANGE: keep expanding after a new leaf until the
            # radar adapter reports no actions at the scheduling boundary.
            while True:
                while node.expanded():
                ...
                reward = models.support_to_scalar(reward, self.config.support_size).item()
                node.expand(
                    # RADAR ADAPTER CHANGE: legality changes with the latent prefix.
