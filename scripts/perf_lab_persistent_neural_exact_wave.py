...
def main() -> None:
    ...
    report = {
        ...
        "seed": int(args.seed),
        "one_time_root_setup_ms": float(one_time_setup_ms),
        "waves": [],
    }
            ...
            report["waves"].append(
                {
                    ...
                    "persistent_neural_expand": neural_stat,
                    "exact_branch_sim": sim_stat,
                    "persistent_combined_wave": combined_stat,
                    "sim_fraction_of_combined": float(sim_stat["mean_ms"] / max(combined_stat["mean_ms"], 1e-12)),
                    "neural_fraction_of_combined": float(neural_stat["mean_ms"] / max(combined_stat["mean_ms"], 1e-12)),
                }
            )
    finally:
        sim.close()
